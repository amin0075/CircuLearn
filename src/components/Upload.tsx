...
const Upload: React.FC<IProps> = (props) => {
  ...
  const { variant = 'image', label, setFile, file, image = '/images/user-profile.jpeg' } = props;
    ...
    <div className="flex-1 flex flex-col items-start gap-3">
      ...
      {variant === 'image' ? (
        <div
          ...
          style={{
            backgroundImage: selectedImage ? `url(${selectedImage})` : `url(${image})`,
          }}
        ...
        <div
          ...
          <Typography variant="caption" className="text-darkBlue-100">
            {file
              ? `${t('fileIsSelected')}: (${fileName.split('\\')[fileName.split('\\').length - 1]})`
              : 'upload'}
          </Typography>
        </div>
      )}
      <input
        accept={
          variant === 'image' ? 'image/png, image/gif, image/jpeg, image/jpg' : 'application/pdf'
        }
        ref={InputRef}
